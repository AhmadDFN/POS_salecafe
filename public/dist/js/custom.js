...
function add_menu(id_menu, nm_menu, harga) {
    ...
    item.find(".price")
        .find("h4")
        .html("<span>Rp</span> " + number_format(harga));
    ...
    if ($("#" + id_menu).length == 0) {
        ...
        $("#" + id_menu)
            .find(".price")
            .find("h4")
            .html("<span>Rp</span> " + number_format(subtotal));
    }
...
function save_transaksi() {
    // Validasi
    if ($("#id_meja").val() == "") {
        showMessage("error", "Maaf Meja Belum Dipilih !!!", "#transaksi");
        return;
    }
    if ($(".detail .detail-item").length == 0) {
        showMessage("error", "Maaf Menu Belum Dipilih !!!", "#transaksi");
        return;
    }

    if ($("#id_member").val() == "") {
        showMessage("error", "Maaf Member Belum Dipilih !!!", "#transaksi");
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.error == 0) {
                ...
                // Cetak nota
                var url =
                    $("#btn-print").attr("data-url") + "/" + data.id_transaksi;
                $("#nota").attr("src", url);
